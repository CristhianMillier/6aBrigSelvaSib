...
public class EscudoBrig extends AppCompatActivity {
    private static final int  PERMISSION_STOREGE_CODE = 1000;
    private Button descEscudo;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_escudo_brig);

        descEscudo = (Button) findViewById(R.id.pdfEscudo);
        descEscudo.setOnClickListener(new View.OnClickListener() {
    ...
    private void descargarEscudo(){
        ...
        File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "Escudo-de-la-6a-Brigada-de-Selva.pdf");
        if (file.exists()){
            ...
            startActivity(Intent.createChooser(intent, "Escudo de la Sexta Brigada de Selva"));
        } else{
            if (isConnected){
                String url = "https://drive.google.com/uc?export=download&id=1o8us7EIPy-mt68fHSCZJ0aZaJS2u8bbg";
                Toast.makeText(this, "Descargando PDF.", Toast.LENGTH_LONG).show();
                DownloadManager downloadManager = (DownloadManager) getSystemService(DOWNLOAD_SERVICE);
                Uri uri = Uri.parse(url);
                DownloadManager.Request request = new DownloadManager.Request(uri);
                request.setTitle("Escudo de la Sexta Brigada de Selva");
                request.setDescription("Por favor, espere un momento...");

                request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "Escudo-de-la-6a-Brigada-de-Selva.pdf");
